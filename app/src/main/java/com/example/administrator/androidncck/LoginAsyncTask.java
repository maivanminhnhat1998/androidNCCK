...
public class LoginAsyncTask extends AsyncTask<String, Boolean, JSONObject>{
    ...
    private ISingin mSingin;
    private Context mContext;
    private ProgressDialog dialog;
    public LoginAsyncTask(Context context,ISingin iSingin,Map<String,String> map){
        this.mMap = map;
        this.mSingin = iSingin;
    ...
    protected void onPostExecute(JSONObject jsonObject) {
        if (jsonObject != null){
            try {
                ...
                if (mResult > 0){
                    mSingin.onLoginSuccess(mMessage);
                }
                else{
                    mSingin.onLoginFail(mMessage);
                }
            } catch (JSONException e) {
                e.printStackTrace();
            }
